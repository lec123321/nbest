...
//5.按照所给的姓名查询学生信息
void PrintOut(SS stu[],char name[],int N);
...
SS* readDataFromFile(int *N)
{
	...
	while ((!feof(fp)))
	{
		...
		//读入文件数据到内存
		fscanf(fp, "%s%s%f%f%f\n", (stu[index].number), (stu[index].name), &stu[index].dailyScore, &stu[index].finalScore,  &stu[index].experimentScore);
...
void calcuScore(SS stu[], int N)
{
	...
	for (int i = 0; i < N; i++)
	{
		...
		stu[i].generalScore = 0.2*stu[i].dailyScore + 0.2*stu[i].experimentScore + 0.8*stu[i].finalScore;
...
//5.查询某个同学的信息
void PrintOut(SS stu[],char name[10],int N)
{
    int i;
    for(i=0;i<N;i++)
    {
       if(stu[i].name[0]==name[0])
        {
            printf("第%d名信息 学号：%s	姓名:%s		总成绩:%4.2f分      实验成绩：%4.2f分      平时成绩：%4.2f分\n", i + 1, &(stu[i].number[0]), &(stu[i].name[0]), stu[i].generalScore,  &stu[i].experimentScore,  stu[i].dailyScore);
            break;
        }
...
int main()
{
	...
	/*-3.计算学生总成绩（总成绩 = 0.3*平时成绩 +0.2*实验成绩+ 0.8*期末成绩)--*/
	calcuScore(pstu, N);
